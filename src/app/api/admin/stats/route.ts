// src/app/api/admin/stats/route.ts — S-20 Admin Dashboard analytics
...
export async function GET() {
  ...
  const [
    ...
    // Users who triggered Pax more than once
    admin.from('pax_triggers').select('user_id').not('state_id_selected', 'is', null),
    // Avg time from conversation creation to close (for average time between match and Close Conversation)
    admin.from('conversations').select('created_at, archived_at').eq('status', 'archived').not('archived_at', 'is', null).limit(500),
    // Screen view counts for drop-off analysis
  ...
  ;(allTriggers || []).forEach((r: any) => {
    if (r.state_id_selected) {
      emotionTally[r.state_id_selected] = (emotionTally[r.state_id_selected] || 0) + 1
    }
  })
  ...
  const mostNotQuite = Object.entries(feedbackByState)
    .sort((a, b) => b[1].notQuite - a[1].notQuite)[0]?.[0] || null

  // Users who triggered Pax more than once
  const userTriggerCounts: Record<string, number> = {}
  ;(paxMoreThanOnce || []).forEach((r: any) => {
    userTriggerCounts[r.user_id] = (userTriggerCounts[r.user_id] || 0) + 1
  })
  ...
  // Average time from match to Close Conversation (in hours)
  ...
  if (closedConvTimings && closedConvTimings.length > 0) {
    ...
    if (validTimings.length > 0) {
      const totalMs = validTimings.reduce((sum: number, c: any) => {
        return sum + (new Date(c.archived_at).getTime() - new Date(c.created_at).getTime())
      }, 0)
      avgTimeToCloseHours = Math.round((totalMs / validTimings.length) / (1000 * 60 * 60) * 10) / 10
    }
  }

  // Drop-off analysis: count unique users who reached each screen
  ...
  return NextResponse.json({
    ...
    convsStartedToday: convsStartedToday || 0,
    paxToday: paxToday || 0,
    closeConvToday: closeConvToday || 0,
    inactivityToday: inactivityToday || 0,
    // Pax performance
    emotionBreakdown: emotionTally,
    feedbackYes,
    feedbackNo,
    feedbackByState,
    mostNotQuite,
    // User patterns
    multipleTriggersCount,
    // Raw
    signupTrend,
