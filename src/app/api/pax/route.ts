...
export async function PATCH(req: NextRequest) {
  ...
  const { data, error } = await supabase
    .from('pax_triggers')
    .update({ feedback_response: feedback_response || null, feedback_open_text: feedback_open_text?.trim().slice(0, 300) || null })
...
export async function GET() {
  ...
  const { data: existing } = await supabase.from('pax_triggers').select('conversation_id')
    .eq('user_id', user.id).eq('trigger_type', 'INACTIVITY').in('conversation_id', inactiveIds)
  ...
  if (toCreate.length > 0) {
    await supabase.from('pax_triggers').insert(toCreate.map(c => ({ user_id: user.id, conversation_id: c.id, trigger_type: 'INACTIVITY' })))
  }

  const { data: pending } = await supabase.from('pax_triggers')
    .select('id, conversation_id, trigger_type, created_at')
    .eq('user_id', user.id).eq('trigger_type', 'INACTIVITY').is('state_id_selected', null)
    .in('conversation_id', inactiveIds).order('created_at', { ascending: true })
