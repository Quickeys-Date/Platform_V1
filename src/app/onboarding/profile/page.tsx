'use client'
// src/app/onboarding/profile/page.tsx — S-04 Profile Setup
...
export default function ProfileSetupPage() {
  ...
  const toggleInterest = (v: string) => {
    setForm(p => ({
      ...p,
      interestedIn: p.interestedIn.includes(v) ? p.interestedIn.filter(x => x !== v) : [...p.interestedIn, v]
    }))
  }
  ...
  return (
    <div className="flex flex-col min-h-svh animate-fade-up">
      ...
      <div className="flex items-center gap-3 px-5 py-3">
        <button onClick={() => step === 2 ? setStep(1) : window.location.href = '/'} className="text-xl">←</button>
                ...
                {GENDERS.map(g => (
                  <button key={g} onClick={() => setForm(p => ({ ...p, gender: g }))}
                    className={`px-4 py-2 rounded-full border-[1.5px] text-sm font-medium transition-colors
                      ${form.gender === g ? 'bg-black text-white border-black' : 'bg-white text-black border-gray-200 hover:border-gray-400'}`}>
                    {g}
                  </button>
                ))}
              </div>
              ...
              <div className="flex flex-wrap gap-2">
                {INTERESTS.map(g => (
                  <button key={g} onClick={() => toggleInterest(g)}
                    className={`px-4 py-2 rounded-full border-[1.5px] text-sm font-medium transition-colors
                      ${form.interestedIn.includes(g) ? 'bg-black text-white border-black' : 'bg-white text-black border-gray-200 hover:border-gray-400'}`}>
                    {g}
                  </button>
                ))}
              </div>
              ...
              <div className="flex flex-wrap gap-2">
                {RADII.map(r => (
                  <button key={r} onClick={() => setForm(p => ({ ...p, radius: r }))}
                    className={`px-4 py-2 rounded-full border-[1.5px] text-sm font-medium transition-colors
                      ${form.radius === r ? 'bg-black text-white border-black' : 'bg-white text-black border-gray-200'}`}>
                    {r}
                  </button>
                ))}
              </div>
