...
export async function GET(req: NextRequest) {
  ...
  let query = supabase.from('conversations')
    ...
    .or(`initiator_id.eq.${user.id},recipient_id.eq.${user.id}`).eq('status', status)

  if (status === 'archived') query = query.order('archived_at', { ascending: false })
  else query = query.order('last_message_at', { ascending: false })

  const { data, error } = await query
  if (error) return NextResponse.json({ error: error.message }, { status: 500 })

  const convs = (data || []).map(c => ({ ...c, other_profile: c.initiator_id === user.id ? c.recipient : c.initiator }))
...
export async function POST(req: NextRequest) {
  ...
  const { data: existing } = await supabase.from('conversations').select('id, status')
    .or(`and(initiator_id.eq.${user.id},recipient_id.eq.${recipient_id}),and(initiator_id.eq.${recipient_id},recipient_id.eq.${user.id})`).single()

  if (existing) return NextResponse.json({ conversation: existing, existing: true })

  const { data, error } = await supabase.from('conversations').insert({ initiator_id: user.id, recipient_id }).select().single()
  if (error) return NextResponse.json({ error: error.message }, { status: 500 })
